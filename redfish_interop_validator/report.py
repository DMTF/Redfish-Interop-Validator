...
def build_service_result_header(service_summary, payload_id, results_id):
  ...
  <div class="resource-card">
    <div class="resource-header">
      <div>
        <div class="resource-uri">Service Resource Requirements</div>
      </div>
...
def build_resource_detail(results_id, results_str, payload_id, payload_str, name_col="Property", message_col="Value"):
...
def html_report(sut: SystemUnderTest, report_dir, time, tool_version, args=None, profile_name=None):
    ...
    """
    file = report_dir / datetime.strftime(time, "RedfishInteropValidatorReport_%m_%d_%Y_%H%M%S.html")
    html = ""
    ...
    # Insert the service results header
    results_id = "resultsService"
    payload_id = "payloadService"
    html += build_service_result_header(
        service_resources_summary,
        payload_id,
        results_id,
    )

    # Insert the resource results details
    results_str = ""
    resources = sorted(list(sut._service_resource_results["Results"]))
    for resource in resources:
        result_class = ""
        raw_val = ""
        value_str = "<div>{}</div>".format(html_mod.escape(str(raw_val)) if raw_val is not None else "")
        message_lines = sut._service_resource_results["Results"][resource]["Message"].split("\n")
        if sut._service_resource_results["Results"][resource]["Result"] == validate.Result.PASS:
            result_class = 'class="res-pass"'
        elif sut._service_resource_results["Results"][resource]["Result"] == validate.Result.WARN:
            result_class = 'class="res-warn"'
            for item in message_lines:
                value_str += '<div class="msg-warn">{}</div>'.format(html_mod.escape(item))
        elif sut._service_resource_results["Results"][resource]["Result"] == validate.Result.FAIL:
            result_class = 'class="res-fail"'
            for item in message_lines:
                value_str += '<div class="msg-fail">{}</div>'.format(html_mod.escape(item))
        elif sut._service_resource_results["Results"][resource]["Result"] == validate.Result.SKIP:
    ...
    for index, uri in enumerate(uris):
        if not sut._resources[uri]["Validated"]:
            ...
            continue
        if (
            sut._resources[uri]["Counts"][validate.Result.PASS.name] == 0
            and sut._resources[uri]["Counts"][validate.Result.SKIP.name] == 0
            and sut._resources[uri]["Counts"][validate.Result.WARN.name] == 0
            and sut._resources[uri]["Counts"][validate.Result.FAIL.name] == 0
        ):
            # Skip resources that have no results; not part of the profile
            continue

        # Get the type info for the URI
        resource_type, resource_version, _ = sut.get_resource_type(uri)
        ...
        for prop in props:
            ...
            value_str = "<div>{}</div>".format(html_mod.escape(str(raw_val)) if raw_val is not None else "")
            message_lines = sut._resources[uri]["Results"][prop]["Message"].split("\n")
            if sut._resources[uri]["Results"][prop]["Result"] == validate.Result.PASS:
                result_class = 'class="res-pass"'
            elif sut._resources[uri]["Results"][prop]["Result"] == validate.Result.WARN:
                result_class = 'class="res-warn"'
                for item in message_lines:
                    value_str += '<div class="msg-warn">{}</div>'.format(html_mod.escape(item))
            elif sut._resources[uri]["Results"][prop]["Result"] == validate.Result.FAIL:
                result_class = 'class="res-fail"'
                for item in message_lines:
                    value_str += '<div class="msg-fail">{}</div>'.format(html_mod.escape(item))
            elif sut._resources[uri]["Results"][prop]["Result"] == validate.Result.SKIP:
                result_class = 'class="res-skip"'
            results_str += "<tr><td>{}</td><td>{}</td><td {}>{}</td></tr>".format(
                prop_str, value_str, result_class, sut._resources[uri]["Results"][prop]["Result"].name
            )
        try:
    ...
    with open(str(file), "w", encoding="utf-8") as fd:
        fd.write(
            build_html_report(
                ...
                tool_repo="DMTF/Redfish-Interop-Validator",
...
def xlsx_report(sut: SystemUnderTest, report_dir, time, tool_version, args=None, profile_name=None):
    ...
    """
    xlsx_file = report_dir / datetime.strftime(time, "RedfishInteropValidatorReport_%m_%d_%Y_%H%M%S.xlsx")
    ...
    result_fill = {
        validate.Result.PASS.name: (_fill(C_PASS_BG), _font(bold=True, color=C_PASS_FG)),
        validate.Result.FAIL.name: (_fill(C_FAIL_BG), _font(bold=True, color=C_FAIL_FG)),
        validate.Result.WARN.name: (_fill(C_WARN_BG), _font(bold=True, color=C_WARN_FG)),
        validate.Result.SKIP.name: (_fill(C_SKIP_BG), _font(color=C_SKIP_FG)),
    }
    ...
    # ════════════════════════════════════════════════════════════════════
    # Sheet 2 — Resource Results
    # ════════════════════════════════════════════════════════════════════
    ws = wb.create_sheet(title="Resources")

    # Column widths: S#, Resource Type, Result, Message
    col_widths = [6, 30, 12, 120]
    ...
    _write_header(
        ws,
        1,
        ["S#", "Resource Type", "Result", "Message"],
    )
    ...
    for resource in resources:
        result = sut._service_resource_results["Results"][resource]["Result"].name
        message = sut._service_resource_results["Results"][resource]["Message"]
        sn_cell = ws.cell(row=row_num, column=1, value=serial_num)
        rtype_cell = ws.cell(row=row_num, column=2, value=resource)
        res_cell = ws.cell(row=row_num, column=3, value=result)
        msg_cell = ws.cell(row=row_num, column=4, value=message)

        # S# cell
        sn_cell.fill = _fill("C7D9F1")
        sn_cell.font = _font(color="0D1B2A")
        sn_cell.alignment = _data()
        sn_cell.border = _border()

        # Resource type cell
        rtype_cell.fill = _fill("C7D9F1")
        rtype_cell.font = _font(color="0D1B2A")
        rtype_cell.alignment = _data()
        rtype_cell.border = _border()
    ...
    col_widths = [6, 55, 14, 18, 30, 35, 50, 12, 120]
    for ci, w in enumerate(col_widths, start=1):
        ws.column_dimensions[get_column_letter(ci)].width = w
    ...
    for uri in uris:
        ...
        # Resource type
        rtype, rversion_str, _ = sut.get_resource_type(uri)
        if rtype is None:
            rtype = "Unknown"
        elif rversion_str:
            rtype += " " + rversion_str
