profile_schema = {
    "$id": "http://redfish.dmtf.org/profiles/v1/RedfishInteroperabilityProfile.v1_10_0.json",
    "$schema": "http://json-schema.org/draft-07/schema",
    "title": "#RedfishInteroperabilityProfile.v1_10_0.RedfishInteroperabilityProfile",
    "copyright": "Copyright 2017-2026 DMTF. For the full DMTF copyright policy, see http://www.dmtf.org/about/policies/copyright",
    "type": "object",
    ...
    "properties": {
        ...
        "ProfileName": {"type": "string", "description": "The name of this Redfish profile."},
        "ProfileVersion": {
            "type": "string",
        ...
        "ProfileType": {
            "type": "string",
            "description": "The type of Redfish profile.",
            "enum": ["Interop", "Product"],
            "enumDescriptions": {
        ...
        "License": {"type": "string", "description": "The license statement for this profile."},
        ...
        "Protocol": {
            ...
            "properties": {
                ...
                "ErrorResponse": {
                    "type": "object",
                    "description": "Requirements related to the error response payload as described by the Redfish Specification.",
                    "additionalProperties": False,
                    "properties": {
                        "ExtendedInfo": {
                            "$ref": "#/definitions/ProtocolRequirement",
                            "description": "Specifies a requirement that the service includes the `@Message.ExtendedInfo` object in error response payloads when applicable.",
                        },
                        "Resolution": {
                            "$ref": "#/definitions/ProtocolRequirement",
                            "description": "Specifies a requirement that the service include the `Resolution` property in error response payloads when applicable.",
                        },
                        "ResolutionSteps": {
                            "$ref": "#/definitions/ProtocolRequirement",
                            "description": "Specifies a requirement that the service include the `ResolutionSteps` property in error response payloads when applicable.",
                        },
                        "ResolutionTypes": {
                            "type": "array",
                            "items": {"type": "string"},
                            "description": "The minimum set of `ResolutionType` values that the service shall support and include when applicable in error response payload.  The values shall be defined by the `ResolutionType` property in the `Message` schema.",
                        },
                    },
                },
            },
        ...
        "Resources": {
            ...
            "patternProperties": {
                "^.*$": {
                    "oneOf": [{"$ref": "#/definitions/ResourceProfile"}, {"$ref": "#/definitions/ResourceUseCases"}]
                }
            },
        },
        "Registries": {
            ...
            "patternProperties": {"^.*$": {"$ref": "#/definitions/RegistryProfile"}},
        },
    },
    "definitions": {
        "ResourceProfile": {
            ...
            "properties": {
                ...
                "ConditionalRequirements": {"type": "array", "items": {"$ref": "#/definitions/Condition"}},
                "PropertyRequirements": {
                    ...
                    "patternProperties": {"^.*$": {"$ref": "#/definitions/PropertyProfile"}},
                },
                "ActionRequirements": {
                    "type": "object",
                    "description": "The action requirements for this Redfish interoperability profile.",
                    "patternProperties": {"^.*$": {"$ref": "#/definitions/ActionProfile"}},
                },
            },
            "additionalProperties": False,
        },
        "ResourceUseCases": {
            "type": "object",
            "properties": {"UseCases": {"type": "array", "items": {"$ref": "#/definitions/ResourceProfile"}}},
            "required": ["UseCases"],
            "additionalProperties": False,
        },
        "PropertyProfile": {
            ...
            "properties": {
                "ReadRequirement": {"$ref": "#/definitions/ReadRequirement"},
                "WriteRequirement": {"$ref": "#/definitions/WriteRequirement"},
                "ReplacedByProperty": {
                    "type": "string",
                ...
                "Priority": {
                    "type": "integer",
                    "minimum": 1,
                    "description": "A value describing the relative importance of this requirement.  This can provide guidance from the profile author on how to prioritize implementation conformance to this profile.  The value shall be a 1-based integer, with 1 being the highest priority.",
                },
                "Purpose": {"$ref": "#/definitions/Purpose"},
                "MinCount": {"$ref": "#/definitions/MinCount"},
                "MinSupportValues": {"$ref": "#/definitions/MinSupportValues"},
        ...
        "ActionProfile": {
            ...
            "properties": {
                ...
                "ActionInfo": {
                    ...
                    "enum": ["Mandatory", "Recommended", "None"],
                    "enumDescriptions": {
            ...
            "properties": {
                ...
                "Messages": {
                    ...
                    "patternProperties": {
                        "^.*$": {
                            "type": "object",
                            "properties": {"ReadRequirement": {"$ref": "#/definitions/ReadRequirement"}},
                            "additionalProperties": False,
                        }
                    },
                },
        ...
        "URIs": {
            "type": "array",
            "description": "An array of URI references to which the `ReadRequirement` and `WriteRequirement` are applied.  The URI values shall follow the resource URI pattern definition specified in the Redfish Specification.  For Product-type profiles, values may take the form of a ECMA-262 regular expression starting with `^` and ending with `$`.",
            "items": {"type": "string", "anyOf": [{"format": "uri-template"}, {"format": "regex"}]},
        },
        "ReadRequirement": {
            "type": "string",
        ...
        "WriteRequirement": {
            ...
            "enum": ["Mandatory", "Supported", "Recommended", "None"],
            "enumDescriptions": {
